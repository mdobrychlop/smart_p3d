...
__license__ = "GPL"
__version__ = "0.1.0"
__maintainer__ = "Mateusz Dobrychlop"
...
def extract_structures(folder, scoretype, representation = "fa", density_map = None, map_threshold = None):
    ...
    for pdbfile in pdb_files:
        ...
        print scorelist
        score = float(scorelist[3]); print "asd", score;
...
if __name__=='__main__':
    ...
    structures  = extract_structures(opts.infolder, opts.score_type, opts.repr, opts.density_map, opts.dmap_threshold)
    ...
    c.iterate_structures(structures, opts.measure,int(opts.threshold),int(opts.struct_nr), opts.score_type, opts.oligos)
    
    c.save_measure_matrix(opts.output)
    c.save_cluster_matrix(opts.output)

    res,clust = c.cluster(int(opts.threshold),int(opts.struct_nr), opts.score_type)
