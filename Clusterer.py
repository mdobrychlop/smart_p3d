...
class Cluster():
    ...
    def iterate_structures(self, structure_set, cutoff,
        ...
        if score_type == "pyry3d":
            structure_set = sorted(
                                   structure_set,
                                   key=lambda struct: struct.score,
                                   reverse=True
                                   )
        elif score_type == "ccc":
            structure_set = sorted(
                                   structure_set,
                                   key=lambda struct: struct.ccc,
                                   reverse=True
                                   )
    ...
    def calculate_rmsd_oligo(self, st1, st2):
        ...
        for chain in st1.structure.get_chains():
            ...
            for refch in ref_list:
                if (len(chain.child_list) == len(list(refch.child_list))):
                    distance = self.calculate_chain_rmsd_matrix(
                                                chain, refch, atomtype="CA")
                    if (min_distance == 0) or (distance < min_distance):
                        min_distance = distance
                        closest = refch
            if None is not closest:
        ...
        for c1, c2 in similarity_map:
            pair_dist, pair_length = self.calculate_chain_rmsd_matrix(
                                                          c1, c2, atomtype="CA"
                                                                     )
            sum_dist += pair_dist
    ...
    def count_values_under_cutoff(self, values_list, cutoff):
        """
        Counts how many value from given value list is lower
        or equal to given cutoff.
        """
        counter = 0
    ...
    def save_measure_matrix(self, outfolder):
        """
        RMSD matrix (or matrix containing other scores)
    ...
    def sort_to_files(self, threshold, clusters, outname, infolder):
        """
        Clusters with number of elements above given threshold
        ...
        self.copy_files_to_separate_folders(
                            clusters, threshold, infolder, outname
                                           )
...
class Cluster_Structure():
    ...
    def write_pdb(self, structure, filename):
        """
        Writing to the pdb_file, saving changed coordinates
        """
...
def write_structure(structure, filename):
        """
        Writing structure to the pdb_file, saving changed coordinated
        Parameters:
        -----------
...
def assign_molweight(atom_id):
    """
    Assigns a molecular weight to each atom in a given structure
    Raises:
    -------
    ...
    for char in atom_id:
        ...
        if atom_name in MOLWEIGHTS.keys():
            ...
            raise PyRyStructureError("Atom not known"+atom_name)
...
if __name__ == '__main__':
    ...
    c.save_measure_matrix(opts.output)
    c.save_cluster_matrix(opts.output)

    res, clust = c.cluster(
                           int(opts.threshold), int(opts.struct_nr),
                           opts.score_type
                          )

    if opts.sort:
        c.sort_to_files(int(opts.sort), clust, opts.output, opts.infolder)

    clustersfile = open(opts.output+"_clusters.txt", "w")
    for el in res:
