    ...
    def find_entities(self, comp_id, chain, resname, resnum, atom):
        ...
        if not model[chain][(' ', resnum, ' ')]:
            ...
            raise RestraintError("The residue %s does not have %s number.\
                                 Please check %s chain"%(resname,resnum, chain))
        if atom:
...
class LogicalRestraint(object):
    ...
    def get_score(self, op):
        ...
        self.symmetry_won = self.elements[0].is_symmetry
        for i in xrange(1, len(self.elements)):
            score = self.scores[i]
            if op(score, result):
                result = score
                self.symmetry_won = self.elements[i].is_symmetry
        return result

    @property
    def is_symmetry(self):
        return self.symmetry_won


class AndRestraint(LogicalRestraint):

    def get_score(self):
        return super(AndRestraint, self).get_score(gt)


class OrRestraint(LogicalRestraint):

    def get_score(self):
        return super(OrRestraint, self).get_score(lt)
...
class SymmetryRestraint(object):
    ...
    def update_score(self, scorerFactory):
        self.scores = []
        for distance in self.distances:
            scorer = scorerFactory.createScorer(distance)
            scorer.score()
            self.scores.append(distance.get_score())
    ...
    def get_score(self):
        ...
        for score in self.scores:
            product *= score
        average = pow(product, 1.0/len(self.scores))
        for score in self.scores:
            result += fabs(score - average)
        return result
