...
from Modules.Trans.Interaction       import ResidueDistanceInteraction, SurfaceAccessInteraction, \
                                            PointDistanceInteraction, SymmetryDistances, SymmetryRestraint, \
...
class Trans(object):
    ...
    def __assign_symmetry_restraint(self, restraint, components, iteration):
        distances = []
        for distance in restraint.distances:
            interaction = SymmetryDistances(iteration)
            interaction.set_di_interaction(distance)
            class RestraintMock(object):
                def __init__(self):
                    self.reslist1, self.reslist2 = distance
            interaction.get_restraints(RestraintMock(), components)
            if not interaction.check_restraints_correctness():
                return None
            distances.append(interaction)
        return SymmetryRestraint(distances)
