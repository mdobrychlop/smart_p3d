...
class Structures(object):
    ...
    def get_structures(self, packed_folder, mode = ""):
        ...
        """
        path = os.getcwd()
        folder_name = self.__untar_struct_folder(packed_folder, mode)
    ...
    def __untar_struct_folder(self, packed_folder, mode):
       ...
       #     rmtree(str(packed_folder[:-4]))
        ...
        fpath = os.path.dirname(os.path.abspath(packed_folder)) #"".join(packed_folder.split("\\")[:-1])
    ...
    def __extract_structures(self, folder_name):
        ...
        try:
            ...
            raise InputError("PyRy3D cannot locate unpacked folder with structures. Please check whether the archive was prepared properly")
...
####################################333
        try: 
            (self.names, self.restraints) = rp.run(restraint_file)
############################################
...
class ShapeDescriptor(object):
    ...
    def __init__(self):
        ...
        self.curve_file  = None               #SAXS curve
    ...
    def get_curve(self, curvefile):
        """
            gather data from .dat file with experiemental curve from SAXS/SANS
        Parameters:
        -----------
            mapfile
        Raises:
        ------
            DensityMapError : if file doesn't exist
        """
        try:    handle = open(curvefile, "r")
        except: raise DensityMapError("File"+str(curvefile)+"doesn't exist")
